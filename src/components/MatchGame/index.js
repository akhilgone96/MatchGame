...
class MatchGame extends Component {
  ...
  generateRandomImage = () => {
    ...
    this.setState({activeImage: imagesList[randomIndex]})
  }
  ...
  updateResults = activeId => {
    const {activeImage} = this.state
    const result =
      activeImage.id === activeId
  ...
  renderGameField = () => {
    const {activeImage, activeTabId} = this.state
    const filteredList = this.getFilteredList(imagesList)
    return (
      <div className="game-view">
        <img className="active-image" alt="match" src={activeImage.imageUrl} />
        ...
        <ul className="images-container">
          {filteredList.map(eachImage => (
            <ImageItem
              ...
              generateRandomImage={this.generateRandomImage}
            />
          ))}
        </ul>
      </div>
    ...
    return (
      <div className="match-game-container">
        ...
        <div className="game-container">
          {isGameOver || timer === 0 ? this.endGame() : this.renderGameField()}
        </div>
      </div>
    )
  }
